...
- [ ] Set `OPENAI_API_KEY`, `MEMORA_EMBEDDING_PROVIDER=openai`, `OPENAI_EMBEDDING_MODEL=text-embedding-3-small`, `MEMORA_DATABASE_URL=sqlite:///./memora.sqlite3`, and `MEMORA_USER_ID=demo-user` in the shell.
- [ ] Generate a private 32-byte `MEMORA_LOCAL_TOKEN` and set it in the same backend shell. Do not commit or display it.
...
- [ ] Open the popup, keep `http://127.0.0.1:8765`, paste the same Memora local token, save, and confirm **Connected**.
