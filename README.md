...
The manifest permits only `http://127.0.0.1/*` and `http://localhost/*`. The extension stores no OpenAI credential.

## Usage

1. Start the backend on `http://127.0.0.1:8765`.
2. Open the Memora popup and keep the default URL and `demo-user`, or save your own user ID.
...
- The explicit `user_id` is a storage scope, not authentication.
